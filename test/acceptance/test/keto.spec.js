...
const baseUrl = 'http://172.17.0.1:8088/readings/keto'
...
describe('Keto', () => {
  describe('POST', () => {
    it('Uploads Readings', async () => {
      ...
      const thirdPost = await fetch(baseUrl, {
        method: 'POST',
        body: JSON.stringify({ created: '2018-04-18 14:14:46.000000', data: 0.3 }),
        headers: { 'Content-Type': 'application/json' }
      })
  ...
  describe('GET', () => {
    it('All Readings', async () => {
      ...
      expect(result[0].data).to.eql(0.4)
      expect(result[1].data).to.eql(0.2)
    })
    ...
    xit('Todays readings', async () => {
      ...
      expect(results[0].data).to.eql(0.4)
      expect(results[1].data).to.eql(0.2)
    })
